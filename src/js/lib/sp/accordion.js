...
// accordion title
const drawer_accordion_title = document.querySelectorAll('.js-drawer-accordion-toggle');

// accordion duration 
const drawer_accordion_duration = 0.4;

//const accordion_duration = 1;
if (drawer_accordion_title.length) {

	for (let i = 0; i < drawer_accordion_title.length; i++) {

		drawer_accordion_title[i].addEventListener('click', function (e) {
 ...
 *
 * @param   {[type]}  active_drawer_accordion_title  [active_drawer_accordion_title description]
 *
 * @return  {[type]}                          [return description]
 */
function accordion_open(active_drawer_accordion_title, i) {

	const active_accordion_contents = active_drawer_accordion_title.parentNode.nextElementSibling;
	let start_accordion_contents_height, active_accordion_contents_height;

	active_drawer_accordion_title.classList.add('is-open');
	...
	animation_tab[i] = gsap.fromTo(active_accordion_contents, {
		...
		duration: drawer_accordion_duration,
 ...
 * @return  {[type]}                    [return description]
 */
function accordion_close(active_drawer_accordion_title, i) {
	...
	active_drawer_accordion_title.classList.remove('is-open')
